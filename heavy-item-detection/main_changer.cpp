...
int row=memory_size*1024/(16*4);
    ...
    int stable_width = row; //number of buckets in each row
    int stable_depth = 4; //number of rows 1
    ...
    HeavyChanger<StableSketch>* heavychangermv = new HeavyChanger<StableSketch>(stable_depth, stable_width, LGN*8);
    for (std::string file; getline(tracefiles, file);) {
        ...
        StableSketch* cursk = (StableSketch*)heavychangermv->GetCurSketch();
        t1 = now_us();
        while(adaptor->GetNext(&t) == 1) {
...
error += hh*1.0/it->second;
    ...
    for (int i = 1; i < numfile; i++) {
        ...
        std::cout << std::setw(20) <<  "Stable-Sketch";
